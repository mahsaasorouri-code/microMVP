...
public class MultiagentGraphSolverGurobiTime extends GRBCallback{
...
//		env.set(GRB.IntParam.Threads, 3);
...
//			model.setCallback(null);
	...
	protected int[][] solveWithCluster(Graph graph, int starts[], int goals[], boolean solveRelaxedFirst, boolean setOptimal, int extraSteps, boolean hasTimeLimit, double timeLimit, int[] cluster) throws GRBException{
		long startTime = System.currentTimeMillis(); 
...
//		env.set(GRB.IntParam.Threads, 3);
		...
		if(solveRelaxedFirst && extraSteps == 0){
			...
			while(true){
				timeLimit = timeLimit - (System.currentTimeMillis() - startTime)/1000.0;
				System.out.print("Trying T = " + startTimeSteps + (hasTimeLimit?(" with max time limit " + timeLimit):"") + " ... ");
				GRBModel model = prepareModelWithCluster(env, graph, starts, goals, false, false, startTimeSteps, hasTimeLimit ? timeLimit : -1, cluster);
				model.relax();
				model.optimize();
				double bestBet = model.get(GRB.DoubleAttr.ObjVal);
				model.dispose();
		...
		while(val != starts.length){
			...
			GRBModel model = prepareModelWithCluster(env, graph, starts, goals, true, setOptimal, startTimeSteps, hasTimeLimit ? timeLimit : -1, cluster);
			expectedBound = starts.length;
//			model.setCallback(this);
			model.optimize();
//			model.setCallback(null);
		...
		for(int a = 0; a < starts.length; a++){
			...
			for(int t = 1; t <= timeSteps; t++){
				...
				for(int rv = 0; rv < rvs.length; rv ++){
					int curV = rvs[rv];
					graph.adjacencySetMap.get(curV).add(curV);//NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN
...
private GRBModel prepareModelWithCluster(GRBEnv env, Graph graph, int starts[], int goals[], boolean integer, boolean setOptimal, int timeSteps, double timeLimit, int[] cluster) throws GRBException{
		// Reset global storage
		edgeVarMap = new TreeMap<Long, GRBVar>();
		vertexTimeInVarVectorMap = new TreeMap<Long, Vector<GRBVar>>();
		vertexTimeOutVarVectorMap = new TreeMap<Long, Vector<GRBVar>>();
		...
		model.update();
		// Set up constraints for Clusters of Robots
		int[] direction = {graph.coll, -graph.coll, 1, -1, 0};
		// int[] direction = {8, -8, 1, -1, 0};
		// int[] direction = {16, -16, 1, -1, 0};
		for(int t = 0; t < timeSteps; t++){
			int apassed = 0;	// For all time Steps
			for(int c = 0; c < cluster.length; c++){		// For all Clusters
				int aPrime = apassed;
				GRBLinExpr[] exprDirAPrime = {null, null, null, null, null};
				for(int d = 0; d < direction.length ; d++){
					for(int curV = 0; curV < graph.vertices.length; curV++){
						int nextV = curV + direction[d];
						if(nextV >= 0 && nextV < graph.vertices.length){
							long robotVVid = getEdgeId(aPrime, curV, nextV, t);
							GRBVar var = edgeVarMap.get(robotVVid);
							if(var != null){
								if(exprDirAPrime[d] == null) 
									exprDirAPrime[d] = new GRBLinExpr();
								exprDirAPrime[d].addTerm(1, var);
							}		
						}		
					}					
				}
				for(int a = apassed + 1; a < apassed + cluster[c]; a++){
					GRBLinExpr[] exprDirAFollower = {null, null, null, null, null};		// For all other robots in this Clusters
					for(int d = 0; d < direction.length; d++){
						for(int curV = 0; curV < graph.vertices.length; curV++){
							int nextV = curV + direction[d];
							if(nextV >= 0 && nextV < graph.vertices.length){
								long robotVVid = getEdgeId(a, curV, nextV, t);
								GRBVar var = edgeVarMap.get(robotVVid);
								if(var != null){
									if(exprDirAFollower[d] == null) 
										exprDirAFollower[d] = new GRBLinExpr();
									exprDirAFollower[d].addTerm(1, var);
								}
							}
						}							
					}
					for(int j = 0; j < 5; j++){
						if(exprDirAPrime[j] != null)
							if(exprDirAFollower[j] != null){
								model.addConstr(exprDirAPrime[j], GRB.EQUAL, exprDirAFollower[j], null);						
							}
					}		
				}
				apassed += cluster[c];
